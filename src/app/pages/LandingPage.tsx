...
>
  ...
  Track Status
      ...
      <div className="fixed bottom-10 right-6 z-50 flex flex-col items-end space-y-4">
          ...
          <div className="bg-white rounded-lg shadow-xl border border-slate-200 w-80 md:w-96 overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200 flex flex-col max-h-[550px]">
            <div className="bg-orange-600 p-4 flex justify-between items-center text-white flex-shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-bold text-sm">Cyber Assistant</h3>
            ...
            <div 
              ...
              className="h-80 md:h-96 bg-slate-50 p-4 overflow-y-auto flex flex-col gap-3"
            >
              {messages.map((msg) => (
                <div 
                  key={msg.id} 
                ...
                >
                  <div 
                    ...
                    } p-3 rounded-xl shadow-sm max-w-[85%] text-sm`}
                  >
                    {msg.content}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2 max-w-[90%]">
                      {msg.options.map((opt) => (
                        <button
                          key={opt}
                          onClick={async () => {
                              const userMsg: Message = { id: Date.now(), content: opt, sender: 'user' };
                              setMessages(prev => [...prev, userMsg]);
                              
                              try {
                                  const response = await chatBotService.chat(opt);
                                  ...
                                  const errorMsg: Message = {
                                     id: Date.now() + 1,
                                     content: error.response?.data?.message || "Failed to get response from server.",
                                     sender: 'bot'
                                   };
                                   setMessages(prev => [...prev, errorMsg]);
                              }
                          }}
                          className="bg-white text-orange-600 border border-orange-200 hover:bg-orange-50 text-xs px-3 py-1.5 rounded-full transition-colors shadow-sm font-medium"
                        >
                          {opt}
                        </button>
                      ))}
            ...
            <form onSubmit={async (e) => {
              ...
              try {
                const response = await chatBotService.chat(currentInput);
                const botMsg: Message = { 
                  id: Date.now() + 1, 
                  content: response.data, 
                  sender: 'bot'
                ...
                setMessages(prev => [...prev, botMsg]);
                 ...
                 const errorMsg: Message = {
                   id: Date.now() + 1,
                   content: error.response?.data?.message || "An error occurred while connecting to the assistant.",
                   sender: 'bot'
                 };
                 setMessages(prev => [...prev, errorMsg]);
              }
            }} className="p-3 bg-white border-t border-slate-100 flex-shrink-0">
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 text-sm border border-slate-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500"
                />
                <Button type="submit" size="sm" className="bg-orange-600 hover:bg-orange-700 px-3">
        ...
        <Button 
          onClick={toggleChat}
          className={`rounded-full h-14 w-14 shadow-lg p-0 flex items-center justify-center transition-all duration-300 hover:scale-110 ${
            isChatOpen ? 'bg-slate-800 hover:bg-slate-900 rotate-90' : 'bg-orange-600 hover:bg-orange-700'
          }`}
        >
